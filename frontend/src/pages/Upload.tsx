      ...
      <div className="glass-ivory border border-stone-200/80 rounded-2xl p-5 sm:p-6 shadow-sm">
        ...
        <div className="mt-4 pt-3.5 border-t border-stone-200/70 grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-[11px] font-mono">
          ...
          <div className="flex items-center gap-2 text-stone-600 bg-stone-50/80 px-3 py-1.5 rounded-lg border border-stone-200/60">
            ...
            <span className="truncate">Hashing: <strong className="text-indigo-700">SHA-256 Merkle</strong></span>
          </div>
          <div className="flex items-center gap-2 text-stone-600 bg-stone-50/80 px-3 py-1.5 rounded-lg border border-stone-200/60">
        ...
        <div className="glass-ivory border border-emerald-300 rounded-2xl p-6 sm:p-7 shadow-lg space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-stone-200/70">
            <div className="flex items-center gap-2 text-emerald-700 font-serif-judicial font-bold text-lg">
              ...
              <span>{result.anchored_on_chain ? 'Evidence Anchored on Polygon & Database' : 'Evidence Successfully Ingested & Verified'}</span>
            </div>
            <div className="flex items-center gap-2">
              {result.anchored_on_chain ? (
                <>
                  <span className="px-2.5 py-1 bg-purple-100 text-purple-900 text-[10px] font-mono font-bold rounded-full border border-purple-200 flex items-center gap-1">
                ...
                <span className="px-2.5 py-1 bg-emerald-100 text-emerald-800 text-[10px] font-mono font-bold rounded-full border border-emerald-200 flex items-center gap-1">
          ...
          {/* Dual Column Layout: Thumbnail Card + Forensic Commitments */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="space-y-2">
              ...
              <div className="bg-white border border-stone-200 rounded-xl overflow-hidden shadow-xs p-2 aspect-[4/5] flex items-center justify-center">
                {result.thumbnail_svg ? (
                  ...
                  <div className="text-center p-4 text-xs font-mono text-stone-400">
                    Thumbnail Stored in Database
                  </div>
                )}
              </div>
              <div className="text-[10px] font-mono text-center text-stone-500">
                SVG Tokenized & Cryptographically Bound
              </div>
            </div>

            {/* Right: Cryptographic Commitments & DID / CRD Numbers */}
            <div className="md:col-span-2 space-y-2.5 text-xs font-mono bg-white p-4 sm:p-5 rounded-xl border border-stone-200 text-stone-700 shadow-sm">
              {/* Document ID */}
              ...
              {/* DID Number */}
              <div className="flex items-center justify-between py-1 border-b border-stone-200/60">
                <span className="text-stone-500 font-semibold">DID Number:</span>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-crimson-800 text-[11px] truncate max-w-[220px]" title={result.did}>
                    {result.did}
                  </span>
                  <button
                    onClick={() => copyToClipboard(result.did, 'did')}
                    className="text-stone-400 hover:text-stone-700 p-0.5 cursor-pointer"
                    title="Copy DID Number"
                  >
                    {copiedField === 'did' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                  </button>
                </div>
              </div>
              ...
              {/* CRD / CID Number */}
              <div className="flex items-center justify-between py-1 border-b border-stone-200/60">
                <span className="text-stone-500 font-semibold">CRD / CID Number:</span>
                <div className="flex items-center gap-2">
                  <span className="font-bold text-indigo-800 text-[11px] truncate max-w-[220px]" title={result.crd}>
                    {result.crd}
                  </span>
                  <button
                    onClick={() => copyToClipboard(result.crd, 'crd')}
                    className="text-stone-400 hover:text-stone-700 p-0.5 cursor-pointer"
                    title="Copy CRD Number"
                  >
                    {copiedField === 'crd' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                  </button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between py-1 border-b border-stone-200/60">
                ...
                <div className="flex items-center gap-2">
                  <span className="text-emerald-700 font-bold text-[11px]">
                    {result.chunk_merkle_root.slice(0, 16)}...{result.chunk_merkle_root.slice(-8)}
                  </span>
                  <button
              ...
              {/* 256KB Chunks */}
              <div className="flex items-center justify-between py-1 border-b border-stone-200/60">
                <span className="text-stone-500 font-semibold">256KB Chunks:</span>
                <span className="font-bold text-stone-900">{result.chunk_count} Chunks (Indexed in Database)</span>
              </div>

              {/* Polygon Anchor - Only shown when actually anchored on Polygon EVM */}
              {result.anchored_on_chain && result.ledger_tx_id ? (
          ...
          {/* Primary Evidence Document Content Preview Block */}
          <div className="space-y-1.5 pt-2 border-t border-stone-200">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-stone-800 uppercase tracking-wider font-mono flex items-center gap-2">
                <FileText className="w-4 h-4 text-crimson-800" />
                <span>Evidence Document Content ({result.filename})</span>
              </span>
              {result.ocr_text && (
                <button
                  type="button"
                  onClick={() => copyToClipboard(result.ocr_text, 'ocr_text')}
                  className="text-xs text-crimson-800 hover:underline font-mono font-semibold flex items-center gap-1 cursor-pointer"
                >
                  {copiedField === 'ocr_text' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                  <span>{copiedField === 'ocr_text' ? 'Copied' : 'Copy Evidence Text'}</span>
                </button>
              )}
            </div>
            <div className="p-3.5 bg-white rounded-xl border border-stone-300 text-stone-900 font-mono text-xs max-h-44 overflow-y-auto whitespace-pre-wrap select-text shadow-inner">
              {result.ocr_text || `[Binary / Encrypted Payload: ${result.filename} (${(result.size_bytes / 1024).toFixed(1)} KB)]\nSHA-256 Digest: ${result.content_hash}`}
            </div>
          </div>
          ...
          <div className="pt-3 flex flex-wrap items-center justify-between gap-3 border-t border-stone-200">
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>Verify & Download Evidence</span>
              </button>
