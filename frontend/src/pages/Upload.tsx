        ...
        <div className="glass-ivory border border-emerald-300 rounded-2xl p-6 sm:p-7 shadow-lg space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-stone-200/70">
            <div className="flex items-center gap-2 text-emerald-700 font-serif-judicial font-bold text-lg">
              ...
              <span>Evidence Anchored on Polygon & Supabase</span>
            </div>
              ...
              <div className="bg-white border border-stone-200 rounded-xl overflow-hidden shadow-xs p-2 aspect-[4/5] flex items-center justify-center">
                {result.thumbnail_svg ? (
                  ...
                  <div className="text-center p-4 text-xs font-mono text-stone-400">
                    Thumbnail Stored in Supabase
                  </div>
                )}
              </div>
              <div className="text-[10px] font-mono text-center text-stone-500">
                SVG Tokenized & Saved to Supabase
              </div>
            </div>

            {/* Right: Cryptographic Commitments & DID / CRD Numbers */}
            <div className="md:col-span-2 space-y-2.5 text-xs font-mono bg-parchment-50 p-4 sm:p-5 rounded-xl border border-stone-200 text-stone-700">
              {/* Document ID */}
              ...
              <div className="flex items-center justify-between py-1 border-b border-stone-200/60">
                ...
                <span className="font-bold text-stone-800">{result.chunk_count} Chunks (Indexed in Supabase)</span>
              </div>

              {/* Supabase Storage */}
              <div className="flex items-center justify-between py-1 border-b border-stone-200/60">
                <span className="text-stone-500 font-semibold">Supabase Storage:</span>
                <span className="text-[11px] text-indigo-700 font-semibold truncate max-w-[200px]" title={result.storage_path}>
                  {result.storage_path}
                </span>
              </div>
          ...
          {/* OCR Extracted Text Preview Block */}
          {result.ocr_text && (
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-bold text-stone-700 uppercase tracking-wider font-mono">
                  Extracted OCR Plaintext (Saved in Supabase chunks & searchable)
                </span>
                <button
                  type="button"
                  onClick={() => copyToClipboard(result.ocr_text, 'ocr_text')}
                  className="text-xs text-crimson-800 hover:underline font-mono font-semibold flex items-center gap-1"
                >
                  {copiedField === 'ocr_text' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5" />}
                  <span>{copiedField === 'ocr_text' ? 'Copied' : 'Copy OCR'}</span>
                </button>
              </div>
              <div className="p-3 bg-white rounded-xl border border-stone-200 text-stone-800 font-mono text-xs max-h-36 overflow-y-auto whitespace-pre-wrap select-text shadow-inner">
                {result.ocr_text}
              </div>
            </div>
          )}
