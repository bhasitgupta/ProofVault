  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      ...
      try {
        ...
        console.warn('Lenis window init fallback', e);
      }
      ...
      const scrollerContent = scroller.querySelector('.scroll-stack-inner') as HTMLElement | null;

      const lenis = new Lenis({
        wrapper: scroller,
        content: scrollerContent || undefined,
        duration: 1.2,
        easing: (t: number) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
        smoothWheel: true,
        touchMultiplier: 2,
