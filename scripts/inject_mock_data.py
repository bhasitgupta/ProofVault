"""
Script: inject_mock_data.py
Injects comprehensive mock cases, assignments, and realistic electronic evidence
corpus documents through the full SDMS cryptographic ingestion pipeline.
"""
import os
import sys
import asyncio
...
os.chdir(BACKEND_DIR)  # Ensure relative paths match the running backend service
...
from app.core.constants import UserRole, ClassificationLevel, DocumentType
...
# ── 3. Mock Case Assignments ──────────────────────────────────────────────────
MOCK_ASSIGNMENTS = [
    # Investigator Sharma (CONFIDENTIAL clearance)
    ("USR-101", "CASE-101"),
    ("USR-101", "CASE-102"),
    ("USR-101", "CASE-105"),
    ("USR-101", "CASE-311"),

    # Forensic Analyst Dr. Ananya (SECRET clearance)
    ("USR-102", "CASE-101"),
    ("USR-102", "CASE-102"),
    ("USR-103", "CASE-103"),
    ("USR-102", "CASE-103"),
    ...
    # Legal Officer Prosecutor Verma (CONFIDENTIAL clearance)
    ("USR-103", "CASE-101"),
    ("USR-103", "CASE-102"),
    ("USR-103", "CASE-104"),
    ("USR-103", "CASE-311"),
    ...
    # SP Kapoor (SUPERVISOR - Full oversight across all 9 cases)
    ("USR-104", "CASE-101"),
    ("USR-104", "CASE-102"),
    ("USR-104", "CASE-103"),
    ("USR-104", "CASE-104"),
    ...
    # Defense Lawyer Advani (RESTRICTED clearance)
    ("USR-105", "CASE-102"),
    ("USR-105", "CASE-104"),
    ("USR-105", "CASE-418"),
...
# ── 4. Mock Evidence Corpus ───────────────────────────────────────────────────
MOCK_DOCUMENTS = [
    # CASE-101: Hawala Breach
    {
...
async def inject_all_mock_data():
    print("=" * 60)
    print("      SDMS — Ingesting Comprehensive Mock Case Dataset     ")
    print("=" * 60)
    ...
    async with async_session_factory() as session:
        # 2. Inject Cases
        print("\n[*] Step 1/3: Seeding & Updating Cases...")
        for case_data in MOCK_CASES:
            existing = await session.get(Case, case_data["case_id"])
            if not existing:
                c = Case(
                    case_id=case_data["case_id"],
                    title=case_data["title"],
                    description=case_data["description"],
                    classification_ceiling=case_data["classification_ceiling"],
                    owning_msp=case_data["owning_msp"],
                    status=case_data["status"]
                )
                session.add(c)
                print(f"  [+] Created Case: {case_data['case_id']} ({case_data['classification_ceiling']}) — {case_data['title'][:40]}...")
            else:
                existing.title = case_data["title"]
                existing.description = case_data["description"]
                existing.classification_ceiling = case_data["classification_ceiling"]
                existing.owning_msp = case_data["owning_msp"]
                existing.status = case_data["status"]
                print(f"  [✓] Verified Case: {case_data['case_id']} ({case_data['classification_ceiling']})")
        await session.commit()

        # 3. Inject Users
        print("\n[*] Step 2/3: Seeding & Verifying Official Accounts...")
        ...
        for u_data in MOCK_USERS:
            ...
            if not existing:
                ...
                print(f"  [✓] Verified User: {u_data['username']} ({u_data['role']})")
        await session.commit()

        # 4. Inject Assignments
        print("\n[*] Step 3/3: Seeding & Updating Live Case Assignments...")
        for user_id, case_id in MOCK_ASSIGNMENTS:
            stmt = select(Assignment).where(Assignment.user_id == user_id, Assignment.case_id == case_id)
            res = await session.execute(stmt)
            assign = res.scalar_one_or_none()
            if not assign:
                a = Assignment(
                    id=f"ASG-{user_id[-3:]}-{case_id[-3:]}",
                    user_id=user_id,
                    case_id=case_id,
                    is_active=True
                )
                session.add(a)
                print(f"  [+] Assigned {user_id} -> {case_id}")
            else:
                assign.is_active = True
        await session.commit()

        # 5. Ingest Evidence Documents through real 14-step pipeline
        print("\n[*] Step 4/4: Ingesting Evidence Documents via Cryptographic Pipeline...")
        ingested_count = 0
        for doc in MOCK_DOCUMENTS:
            try:
                ...
                ingested_count += 1
                print(f"  [✓] Ingested: {doc['case_id']} | {doc['filename']} ({doc['classification']}) -> Doc ID: {res['doc_id'][:8]}... | TX: {res['ledger_tx_id'][:16]}...")
            except Exception as e:
                print(f"  [!] Skipped/Error {doc['filename']}: {e}")

    print("\n" + "=" * 60)
    print(f"[✓] Injection Complete! Ingested {ingested_count} evidence records across 9 mock cases.")
    print("=" * 60)

if __name__ == "__main__":
    asyncio.run(inject_all_mock_data())
